...
run.wbs <- function(pro, ...){
  require(wbs)
  param.list <- list(...)
  each.chrom(pro, function(chr) {
    Y <- chr$logratio
    if(length(Y) < 2){
      mean.mat <- matrix(1, 1, length(Y))
    }else{
      set.seed(1)
      w <- wbs(Y)
      arg.list <- c(list(w), param.list)
      result.list <- do.call(changepoints, arg.list)
      list.or.na <- result.list$cpt.th
      change.list <- if(is.list(list.or.na)) {
        list.or.na
      }else{
        list(integer())
      }
      mean.mat <- matrix(1, length(change.list), length(Y))
      for (param.i in seq_along(change.list)) {
        changes <- sort(change.list[[param.i]])
...
### Run the cghseg.k penalized model selection procedure.
penalized.mean.mat <- function(Y, seg.mean.mat, lambda.vec){
  stopifnot(is.numeric(Y))
  stopifnot(is.numeric(seg.mean.mat))
  stopifnot(is.matrix(seg.mean.mat))
  stopifnot(ncol(seg.mean.mat) == length(Y))
  stopifnot(is.numeric(lambda.vec))
  max.segments <- nrow(seg.mean.mat)
  rss.vec <- rep(NA, max.segments)
  for(n.segments in 1:max.segments){
    residual.vec <- Y - seg.mean.mat[n.segments, ]
    rss.vec[n.segments] <- sum(residual.vec * residual.vec)
  }
  pen.mean.mat <- matrix(NA, length(lambda.vec), length(Y))
  for(lambda.i in seq_along(lambda.vec)){
    lambda <- lambda.vec[[lambda.i]]
    beta <- lambda * length(Y)
    pen.vec <- beta * 1:n.segments
    selected.segments <- which.min(rss.vec + pen.vec)
    pen.mean.mat[lambda.i, ] <- seg.mean.mat[selected.segments, ]
  }
  pen.mean.mat
}  

smuce.q.vec <-
  unique(c(10^seq(-3, -1, l=10),
           seq(0.2, 30, by=0.2),
           seq(40, 200, by=10)))

run.smuce <- function(pro, param.vec, param.name){
  require(stepR)
  each.chrom(pro, function(chr){
    Y <- chr$logratio
    mean.mat <- matrix(NA, length(param.vec), length(Y))
    for(param.i in seq_along(param.vec)){
      param.val <- param.vec[[param.i]]
      fit <- if(is.na(param.val)){
        smuceR(Y) # for default version.
      }else{
        param.list <- list(Y, param.val)
        names(param.list) <- c("y", param.name)
        do.call(smuceR, param.list)
      }
      seg.df <- with(fit, data.frame(leftEnd, rightEnd, value))
      for(segment.i in 1:nrow(seg.df)){
        seg <- seg.df[segment.i, ]
        mean.mat[param.i, seg$leftEnd:seg$rightEnd] <- seg$value
      }
    }
    mean.mat
  })
...
l2vals=c(1e-5,1e-4,1e-3,5e-3,1e-2,2e-2,5e-2,
  ...
  },smuce.default=function(pro, alpha.vals=NA){
    run.smuce(pro, NA)
  },smuce.alpha=function(pro,
      alpha.vals=unique(c(10^seq(-10, -1, l=60), seq(0.1, 0.9, by=0.05)))){
    run.smuce(pro, alpha.vals, "alpha")
  },smuce.q=function(pro, q.vals=smuce.q.vec){
    run.smuce(pro, q.vals, "q")
  },smuce.penalized=function(pro, lambda.vec=10^c(seq(-8,1,l=100))){
    require(stepR)
    each.chrom(pro, function(chr){
      Y <- chr$logratio
      q.mean.mat <- matrix(NA, length(smuce.q.vec), length(Y))
      q.changes.vec <- rep(NA, length(smuce.q.vec))
      for(param.i in seq_along(smuce.q.vec)){
        param.val <- smuce.q.vec[[param.i]]
        fit <- smuceR(Y, q=param.val)
        seg.df <- with(fit, data.frame(leftEnd, rightEnd, value))
        for(segment.i in 1:nrow(seg.df)){
          seg <- seg.df[segment.i, ]
          q.mean.mat[param.i, seg$leftEnd:seg$rightEnd] <- seg$value
        }
        q.changes.vec[[param.i]] <- sum(diff(q.mean.mat[param.i, ]) != 0)
      }
      q.segments.vec <- q.changes.vec + 1
      max.segments <- 20
      seg.mean.mat <- matrix(mean(Y), max.segments, length(Y))
      for(n.segments in 2:max.segments){
        q.row <- which(q.segments.vec == n.segments)[1]
        seg.mean.mat[n.segments, ] <- q.mean.mat[q.row, ]
      }
      penalized.mean.mat(Y, seg.mean.mat, lambda.vec)
    })
  },wbs.penalized=function(pro, lambda.vec=10^c(seq(-8,1,l=100))){
    library(wbs)
    each.chrom(pro, function(chr){
      Y <- chr$logratio
      if(length(Y) < 4){
    ...
    run.wbs(pro, th.const=th.const)
  },wbs.th=function(pro, th=c(0, 10^seq(-1, 2, l=200))){
    run.wbs(pro, th=th)
