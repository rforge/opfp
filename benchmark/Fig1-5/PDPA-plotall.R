...
PDPAplotallin<-function(y,maxseg,timestep,seg,mu.min,mu.max){
  n<-length(y)
  S1<-0
  for(i in 1:n){
    S1[i+1]<-S1[i]+y[i]
  ...
  for (m in 2:maxseg){
    ...
    for (j in m:n){
      ...
      for(kk in (1:(j-1))[-OLOCdash]){
        lines(x,a[kk]*x^2+b[kk]*x+c[kk],col="antiquewhite3")
      }
    for(kk in OLOCdash){
      	if(kk==OLOCdash[1]){
      		lines(x,a[kk]*x^2+b[kk]*x+c[kk],col=co[1])
      	}else{
